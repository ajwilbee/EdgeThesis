function [gbp, global_best_fitness,local_best_position,local_best_fitness]= myFuzzyPSO(imgGrey, Img, coef, iter, cc)
...
for i = 1:n
    temp = (fuzzy_fitness(imgGrey,Img,current_position(:,i)));
...
%         end
    ...
    if(size(local_best_position,1) > 1)
        if(local_best_position(3,:)>511)
            test = 0;
        end
    
    elseif local_best_position > 511
        test = 0;
